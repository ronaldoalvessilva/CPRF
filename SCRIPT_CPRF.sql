...
IF NOT EXISTS (SELECT NULL FROM SYSOBJECTS WHERE ID = OBJECT_ID('BANCOS_CONTAS'))
   CREATE TABLE BANCOS_CONTAS
	...
	OperacaoBanco VARCHAR(50) NULL,
	IdForn INT NULL,
	TipoFavorecido VARCHAR(60) NULL,
...
IF NOT EXISTS (SELECT NULL FROM SYSOBJECTS WHERE ID = OBJECT_ID('MOVIMENTO_BANCARIO'))
   CREATE TABLE MOVIMENTO_BANCARIO
   (
   IdMovBanc INT IDENTITY (1,1) PRIMARY KEY NOT NULL,
   StatusRegistro VARCHAR(80) NULL,
   DataRegistro DATETIME NULL,
   TipoPessoa VARCHAR(50) NULL,
   IdBanco INT FOREIGN KEY REFERENCES BANCOS_CONTAS(IdBanco) NOT NULL,
   Depositante VARCHAR(100) NULL,      
   ValorDeposito REAL NULL,
   TipoDeposito VARCHAR(80) NULL,
   Observacao VARCHAR(MAX) NULL,
   UsuarioInsert VARCHAR(50) NULL,
   UsuarioUp VARCHAR(50) NULL,	
   DataInsert VARCHAR(20) NULL,
...
IF NOT EXISTS (SELECT NULL FROM SYSOBJECTS WHERE ID = OBJECT_ID('ESTORNO_BANCARIO'))
   CREATE TABLE ESTORNO_BANCARIO
   (
   IdEstBanc INT IDENTITY (1,1) PRIMARY KEY NOT NULL,
   StatusEstorno VARCHAR(80) NULL,
   DataEstorno DATETIME NULL,
   TipoPessoa VARCHAR(50) NULL,
   IdBanco INT FOREIGN KEY REFERENCES BANCOS_CONTAS(IdBanco) NOT NULL,
   Depositante VARCHAR(100) NULL,      
   ValorEstorno REAL NULL,
   TipoEstorno VARCHAR(80) NULL,
   Observacao VARCHAR(MAX) NULL,
   UsuarioInsert VARCHAR(50) NULL,
   UsuarioUp VARCHAR(50) NULL,	
   DataInsert VARCHAR(20) NULL,
...
-- CAMPO DA TABELA MOVIMENTACAO_CONTAS_PAGAR_RECEBER PARA NÃO TRAZER AS CONTAS PAGAS NA PESQUISA.

IF NOT EXISTS(SELECT 1 FROM SYSCOLUMNS WHERE ID = OBJECT_ID('MOVIMENTO_CONTAS_PAGAR_RECEBER') AND NAME = 'ContaBaixada')
   ALTER TABLE MOVIMENTO_CONTAS_PAGAR_RECEBER ADD ContaBaixada VARCHAR(3) NULL

-- ADICIONADO CAMPO PARA TIPO DE CENTRO DE  CUSTO

IF NOT EXISTS(SELECT 1 FROM SYSCOLUMNS WHERE ID = OBJECT_ID('CENTRO_CUSTO') AND NAME = 'TipoCentroCusto')
   ALTER TABLE CENTRO_CUSTO ADD TipoCentroCusto VARCHAR(30) NULL